...
[1;2;3] >>= (fun x -> [x; x+1; x+2]) >>= (fun x -> [-x; x])
...
let rec fac (n : int) = writer {
    if n = 0 
    then return 1
    else let! n1 = (n - 1, n.ToString() + " minus 1;")
         let! res = fac n1
         let! mult = (res * n, res.ToString() + " mul " + n.ToString())
         return mult
}
fac 5
